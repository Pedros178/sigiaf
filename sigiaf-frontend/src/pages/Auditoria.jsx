...
export default function Auditoria() {
  ...
  const exportarExcel = () => {
    const catNombre = categorias.find(c => c.id_categoria === parseInt(filtroCategoria))?.nombre || 'Todas';
    const datos = conEstado.map(a => ({
      'Código QR':        a.codigo_qr,
      'SKU':              a.sku || '',
      'Nombre':           a.nombre,
      'Categoría':        a.categoria || '',
      'Ubicación':        a.ubicacion || '',
      'Estado sistema':   a.estado,
      'Estado auditoría': a.estadoAudit,
      'Notas':            notas[a.id_activo] || '',
    }));
    // Resumen en hoja aparte
    const resumen = [
      { 'Campo': 'Fecha auditoría',  'Valor': new Date().toLocaleString('es-MX') },
      { 'Campo': 'Categoría filtro', 'Valor': catNombre },
      { 'Campo': 'Ubicación filtro', 'Valor': filtroUbicacion || 'Todas' },
      { 'Campo': 'Total activos',    'Valor': stats.total },
      { 'Campo': 'Verificados',      'Valor': stats.verificado },
      { 'Campo': 'Faltantes',        'Valor': stats.faltante },
      { 'Campo': 'Pendientes',       'Valor': stats.pendiente },
      { 'Campo': 'Avance (%)',       'Valor': progreso },
    ];
    const wb   = XLSX.utils.book_new();
    const ws1  = XLSX.utils.json_to_sheet(datos);
    ws1['!cols'] = [22,14,28,16,18,16,16,30].map(w=>({wch:w}));
    XLSX.utils.book_append_sheet(wb, ws1, 'Detalle auditoría');
    const ws2  = XLSX.utils.json_to_sheet(resumen);
    ws2['!cols'] = [22,28].map(w=>({wch:w}));
    XLSX.utils.book_append_sheet(wb, ws2, 'Resumen');
    XLSX.writeFile(wb, `auditoria-${new Date().toISOString().split('T')[0]}.xlsx`);
    toast.success('Excel de auditoría generado');
  };
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '16px', alignItems: 'start' }}>
            ...
            <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', gap: '10px', minWidth: '200px' }}>
              ...
              <button onClick={exportarPDF} style={{ ...btnPrimary, background: '#7c3aed', fontSize: '13px' }}>📄 Exportar PDF</button>
              <button onClick={exportarExcel} style={{ ...btnPrimary, background: '#15803d', fontSize: '13px' }}>📊 Exportar Excel</button>
            </div>
          </div>
        </>
      )}
